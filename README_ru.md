...
* `ui:show_grid(title, points, [show_grid], [folded_string], [copyright])` - показывает график, title - название графика, points - таблица таблиц координат, show\_grid - флага показа сетки, folded\_string - строка для свернутого состояния (иначе будет показано название), copyright - строка, отображаемая в правом нижнем углу;
